...
function App(){
  ...
  return <main>
    ...
    <section className="grid">{shown.map(a=><article key={a.id} onClick={()=>setSelected(a)}><div className="poster">{a.image?<img loading="lazy" src={a.image} alt={`${a.title} cover`}/>:<div className="fallback"><b>{a.title.slice(0,1)}</b><span>{a.title}</span></div>}<span className="badge">{a.status||"Uncategorized"}</span>{isRecentlyUpdated(a)&&<span className="newBadge">NEW</span>}</div><h3>{a.title}</h3><div className="rating">{stars(a.score)}</div><small>{a.episodes?`${a.episodes} eps`:""}{a.latestEpisodeYear?` · latest ${a.latestEpisodeYear}`:""}</small></article>)}</section>
