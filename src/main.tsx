...
type Anime={id:string,title:string,status:string,episodes:number|null,score:number|null,genre:string,studio:string,year:number|null,seasons:number|null,latestSeasonYear:number|null,synopsis:string,image:string,metadataSource?:string,malId?:number};
...
const withCover=(a:Anime):Anime=>({...a,seasons:a.seasons??null,latestSeasonYear:a.latestSeasonYear??null,image:coverMap[a.title]||a.image||""});
const initialAnime=()=>repoAnime.map(withCover);
const seasonLabel=(n:number|null)=>n?`${n} season${n===1?"":"s"}`:"";
...
function App(){
  ...
  const newAnime=():Anime=>({id:`anime-${crypto.randomUUID()}`,title:"",status:"Planned",episodes:null,score:null,genre:"",studio:"",year:new Date().getFullYear(),seasons:null,latestSeasonYear:null,synopsis:"",image:""});
  ...
  return <main>
    ...
    <section className="grid">{shown.map(a=><article key={a.id} onClick={()=>setSelected(a)}><div className="poster">{a.image?<img loading="lazy" src={a.image} alt={`${a.title} cover`}/>:<div className="fallback"><b>{a.title.slice(0,1)}</b><span>{a.title}</span></div>}<span className="badge">{a.status||"Uncategorized"}</span></div><h3>{a.title}</h3><div className="rating">{stars(a.score)}</div><small>{a.year||""}{a.episodes?` · ${a.episodes} eps`:""}{a.seasons?` · ${seasonLabel(a.seasons)}`:""}</small></article>)}</section>
    <footer><button onClick={backup}>Export repo data</button></footer>

    {selected&&!edit&&<div className="sheet" onClick={()=>setSelected(null)}><div className="panel" onClick={e=>e.stopPropagation()}><button className="close" onClick={()=>setSelected(null)}>×</button><div className="hero">{selected.image?<img src={selected.image} alt={`${selected.title} cover`}/>:<div className="heroFallback">{selected.title}</div>}</div><h2>{selected.title}</h2><div className="rating big">{stars(selected.score)}</div><p className="meta">{selected.status||"Uncategorized"} · {selected.year||"—"} · {selected.episodes||"—"} episodes{selected.seasons?` · ${seasonLabel(selected.seasons)}`:""}</p>{selected.latestSeasonYear&&selected.latestSeasonYear!==selected.year&&<p className="meta">Most recent season: {selected.latestSeasonYear}</p>}<p>{selected.synopsis||"No synopsis yet."}</p><p className="muted">{selected.genre}<br/>{selected.studio}</p>{editMode&&<div className="actions"><button onClick={()=>setEdit(selected)}>Edit</button><button className="danger" onClick={()=>remove(selected)}>Delete</button></div>}</div></div>}

    {edit&&editMode&&<div className="sheet"><form className="panel form" onSubmit={e=>{e.preventDefault();commit(edit)}}><button type="button" className="close" onClick={()=>setEdit(null)}>×</button><h2>{items.some(x=>x.id===edit.id)?"Edit anime":"Add anime"}</h2>{["title","genre","studio","synopsis"].map(k=><label key={k}>{k[0].toUpperCase()+k.slice(1)}{k==="synopsis"?<textarea value={(edit as any)[k]} onChange={e=>setEdit({...edit,[k]:e.target.value})}/>:<input required={k==="title"} value={(edit as any)[k]} onChange={e=>setEdit({...edit,[k]:e.target.value})}/>}</label>)}<div className="row"><label>Status<select value={edit.status} onChange={e=>setEdit({...edit,status:e.target.value})}><option value="">Uncategorized</option>{["Completed","Paused","Planned"].map(x=><option key={x}>{x}</option>)}</select></label><label>Score<select value={edit.score??""} onChange={e=>setEdit({...edit,score:e.target.value?+e.target.value:null})}><option value="">—</option>{[1,2,3,4,5].map(x=><option key={x} value={x}>{x} ★</option>)}</select></label></div><div className="row"><label>Episodes<input type="number" min="0" value={edit.episodes??""} onChange={e=>setEdit({...edit,episodes:e.target.value?+e.target.value:null})}/></label><label>Original year<input type="number" min="1900" max="2100" value={edit.year??""} onChange={e=>setEdit({...edit,year:e.target.value?+e.target.value:null})}/></label></div><div className="row"><label>Seasons<input type="number" min="1" value={edit.seasons??""} onChange={e=>setEdit({...edit,seasons:e.target.value?+e.target.value:null})}/></label><label>Latest season year<input type="number" min="1900" max="2100" value={edit.latestSeasonYear??""} onChange={e=>setEdit({...edit,latestSeasonYear:e.target.value?+e.target.value:null})}/></label></div><button className="save">Continue in GitHub</button></form></div>}
  </main>
}
