...
export default function App(){
 const[items]=useState<Anime[]>(initialAnime),[selected,setSelected]=useState<Anime|null>(null),[edit,setEdit]=useState<Anime|null>(null),[showAddSearch,setShowAddSearch]=useState(false),[showStats,setShowStats]=useState(false),[editMode,setEditMode]=useState(()=>localStorage.getItem(EDIT_KEY)==="1"),[notice,setNotice]=useState("");
 const filters=useCatalogFilters(items);
 const toggleEdit=()=>{const next=!editMode;setEditMode(next);localStorage.setItem(EDIT_KEY,next?"1":"0");setEdit(null);setShowAddSearch(false);setNotice(next?"Edit mode enabled · GitHub will verify you when you submit":"Edit mode disabled")};
 ...
 return <main>
  ...
  {showAddSearch&&editMode&&<AnimeSearch onSelect={chooseCatalogAnime} onManual={addManual} onClose={()=>setShowAddSearch(false)}/>} 
