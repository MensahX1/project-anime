...
// Remove stale mappings for titles that no longer exist.
...
const workers=Array.from({length:Math.min(12,missing.length)},async()=>{
  while(true){
    ...
    for(let attempt=1;attempt<=4 && !ok;attempt++){
      try{
        const r=await fetch(url,{signal:AbortSignal.timeout(20000),headers:{'User-Agent':'le-anime-cover-export/3.0'}});
